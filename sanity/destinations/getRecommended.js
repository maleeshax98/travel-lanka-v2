...
const getProductType = async (locationRef) => {
  const QUERY = `
    array::unique(
      *[_type == "productType" && $locationRef in cities[]->province._ref]
...
const getActivityTypes = async (locationRef) => {
  const QUERY = `
    array::unique(
      *[_type == "activityType" && $locationRef in cities[]->province._ref]
...
const getRecommendedActivites = async (locationRef, activityCategory) => {
  ...
  const ACTIVITIES_QUERY = `*[_type=='activityType' && defined(slug.current) && $locationRef in cities[]->province._ref && activityCategory->name == $activityCategory ][0...6]{_id, name, slug, mainImage, cities[] -> {name}, link, activityCategory-> {_id, name, slug}, price, address, contactNumbers, introduction}`;
...
const getPlacesTypes = async (locationRef) => {
  const QUERY = `
    array::unique(
      *[_type == "placesType" && $locationRef == city->province._ref]
      .category->name
    )
  `;
...
const getRecommendedPlaces = async (locationRef, placeType) => {
  ...
  const PLACES_QUERY = `*[_type=='placesType' && defined(slug.current) && $locationRef == city->province._ref && category->name == $placeType ][0...6]{_id, name, slug, mainImage, introduction, address,  city->{_id, name, slug, province->{_id, name, slug}}, category->{_id, name, slug}}`;
